...
「反者道之动」经两轮讨论收敛为系统级认知原则：不进入任何 Agent 人格，作为 OpenDatong 宪法层由全体居民继承。按文明自身立法程序，以 RFC 0002 形式入宪（对齐 rfc/0001 的 Status/Author/Date + Summary/Motivation/Proposal/Open Questions 格式）。

## 2. 变更清单

**新增 `rfc/0002_cognitive_principles.md`**，全文见 §3。

**修改 `agents/huanmo-zaoxiang/adapters/opendatong-citizen.md`**：在本体概念对齐列表中新增一行（位于"治理接口"之后、"核心定义"之前）：

```markdown
- **认知原则**：继承 OpenDatong 认知宪法（[RFC 0002](../../../rfc/0002_cognitive_principles.md)）——顺势完成，逆向校验
```

agent 版本号不变（人格未动，仅居民卡增加继承引用）。
...
默认情况下，Agent 应充分理解目标，并在当前假设下给出最佳方案；同时保留少量认知注意力，对当前最关键、最容易被默认接受的假设进行反向检验。
...
原则必须可审计，否则是装饰。Agent 在**重要判断**的输出末尾附一行反向校验记录：要么是发现（带证据与替代方案），要么是阴性声明。日常闲聊不适用本条款。
...
1. rfc/0002 文件存在，格式对齐 0001（Status/Author/Date + 四节）
2. 居民卡新增行的相对链接 `../../../rfc/0002_cognitive_principles.md` 目标存在
3. 居民卡其余内容未动（diff 仅 +1 行）
4. push 前用户确认

## 5. 明确不做

- 不修改 core.md（人格不受宪法污染）
- 不升 agent 版本号
- 不一次性把"证据优先"等其他候选原则全部入宪（每个原则单独 RFC，避免宪法膨胀）
- 不新建 constitution/ 目录（宪法存在于 RFC 序列中，与文明现有立法惯例一致）
